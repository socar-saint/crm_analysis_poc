...
def _sub_all_once(p: re.Pattern[str], repl: str | Callable[[re.Match[str]], str], s: str) -> str:
    """Sub all once."""
    return p.sub(repl, s)


EMAIL_STD = re.compile(
    r"(?i)(?<!\S)[A-Za-z0-9._%+-]{1,64}\s*@\s*"
    r"[\w\-\uAC00-\uD7A3.]{1,253}\s*\.\s*[A-Za-z]{2,24}(?!\S)"
)
...
EMAIL_LOCAL_TOKEN = re.compile(
    r"(?i)(?<!\S)(?=[A-Za-z][A-Za-z0-9._%+-]{2,63}(?!\S))"
    r"[A-Za-z0-9._%+-]+"
)
...
def _mask_email(s: str) -> str:
    ...
    s = _sub_all_once(EMAIL_DOMAIN_ASCII, lambda m: EMAIL_TAG + (m.group("suf") or ""), s)
    s = _sub_all_once(EMAIL_DOMAIN_HANGUL, lambda m: EMAIL_TAG + (m.group("suf") or ""), s)
    s = _sub_all_once(EMAIL_STD, EMAIL_TAG, s)
    s = _sub_all_once(EMAIL_KR_AT, EMAIL_TAG, s)
    s = _sub_all_once(EMAIL_KR_NOAT, EMAIL_TAG, s)
    if EMAIL_CONTEXT_HINT.search(s):
        s = _sub_all_once(EMAIL_LOCAL_TOKEN, EMAIL_TAG, s)
    return s


def _mask_misc(s: str) -> str:
    s = _sub_all_once(PHONE, PHONE_TAG, s)
    s = _sub_all_once(SSN, SSN_TAG, s)
    s = _sub_all_once(CARD, CARD_TAG, s)
    s = _sub_all_once(ADDR, ADDR_TAG, s)
    return s
...
def _starts_with_surname(token: str) -> bool:
    return bool(token) and token[0] in SURNAMES and token[0] not in SAFE_NON_NAMES
...
def _mask_names(s: str) -> str:
    s = _sub_all_once(NAME_LABEL, _replace_name_context, s)
    s = _sub_all_once(NAME_BONIN, _replace_name_context, s)
    s = _sub_all_once(NAME_INTRO, _replace_name_context, s)
    s = _sub_all_once(NAME_NIM, _replace_name_strict, s)
    s = _sub_all_once(NAME_SSI, _replace_name_strict, s)
    return s
